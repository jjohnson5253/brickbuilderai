...
describe('JSON API service contracts', () => {
  ...
  const cases: Array<[string, () => Promise<unknown>, string, unknown, unknown]> = [
    ...
    ['llm render', () => LlmRenderApiService.llmRender('xyz', 'image', 'paint', 'tok', ['back', 'top']), '/llmRender', { xyzrgb_url: 'xyz', reference_image_url: 'image', reference_image_urls: ['back', 'top'], prompt: 'paint' }, { xyzrgb_content: 'xyz', voxel_count: 1, segment_count: 1, model: 'm', applied_rules: [], message: 'ok' }],
  ];
  ...
  it('streams LLM brick-design thinking before returning the result', async () => {
    ...
    await expect(
      LlmRenderApiService.llmRenderStream('xyz', 'image', 'paint', 'tok', thinking, ['back', 'top'])
