...
from dataset.MVNpzDataset import MVNpzDataset, collate_fn
...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                for i, model in enumerate(models):
                    if isinstance(model, ImageConditionModel):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                model = models.pop()
                if isinstance(model, ImageConditionModel):
                    load_model = ImageConditionModel.from_pretrained(os.path.join(input_dir, "transformer"))
    ...
    # dinov2 (uses default HF cache; set HF_HOME or TRANSFORMERS_CACHE to override)
    ...
    dataset = MVNpzDataset(args.split, "train", pos_range=1280, augment=True, cfg=False)
    val_dataset = MVNpzDataset(args.split, "val", pos_range=1280)
    ...
    if args.model_config_name_or_path is None:
        ...
        model = ImageConditionModel.from_pretrained(args.model_config_name_or_path, local_files_only=True)
    ...
    dino_processor, dino_model = accelerator.prepare(dino_processor, dino_model)

    current_time = datetime.now()
    time_string = current_time.strftime("%m-%d-%H-%M")
    if accelerator.is_main_process:
        accelerator.init_trackers("legoace-image-condition", init_kwargs={"wandb": {"id": args.wandb_id+"-"+time_string}})
