...
from utils.infer_utils import make_compare, make_compare_with_repeat
...
if __name__ == "__main__":
    ...
    if cfg.ckpt_dir.startswith("VAST-AI/") or cfg.ckpt_dir.startswith("hf://"):
        ckpt_path = cfg.ckpt_dir
        subfolder = "mv"
    else:
        ckpt_path = Path(cfg.ckpt_dir) / f"checkpoint-{cfg.ckpt_iter}/transformer"
        subfolder = None
    ...
    with tqdm(total=len(test_images) * cfg.repeat) as pbar:
        ...
        for index, image in enumerate(test_images):
            ...
            image.save(image_save_dir / f"{file_name}.png")
            ...
            if cfg.repeat == 1:
                ...
                repeat_images = [image] * cfg.repeat
    ...
    if cfg.repeat == 1:
        images_per_row = 4
        new_image = make_compare(image_save_dir, render_save_dir, images_per_row=images_per_row)
        new_image.save(save_dir / f"compare_{images_per_row}.png")
    else:
        compare_image = make_compare_with_repeat(image_save_dir, render_save_dir, repeat=cfg.repeat, samples_per_row=1)
