...
struct obj2voxel_instance {
    ...
    std::unique_ptr<IVoxelSink> secondaryVoxelSink = nullptr;
    ...
    bool secondarySinkWritable = true;
    bool done = false;
};
...
void voxelizeChunk(obj2voxel_instance &instance, Voxelizer &voxelizer, u32 chunkIndex)
{
    ...
    VXIO_ASSERT_EQ(i, voxelCount);
    {
        ...
        if (instance.secondaryVoxelSink && (instance.secondarySinkWritable &= instance.secondaryVoxelSink->canWrite())) {
            instance.secondaryVoxelSink->write(buffer.get(), voxelCount);
        }
    }
...
[[nodiscard]] obj2voxel_error_t voxelize_specialized(obj2voxel_instance &instance)
{
    ...
    if (instance.secondaryVoxelSink) {
        instance.secondaryVoxelSink->finalize();
    }
...
[[nodiscard]] obj2voxel_error_t voxelize(obj2voxel_instance &instance)
{
    ...
    // Always create a secondary .vox output
    if (instance.output.type == IoType::FILE && instance.output.file.type != FileType::MAGICA_VOX) {
        std::string voxPath = std::string(instance.output.file.path);
        size_t dotPos = voxPath.find_last_of('.');
        if (dotPos != std::string::npos) {
            voxPath = voxPath.substr(0, dotPos) + ".vox";
        } else {
            voxPath += ".vox";
        }
        std::optional<FileOutputStream> voxStream = FileOutputStream::open(voxPath.c_str(), OpenMode::BINARY);
        if (voxStream.has_value()) {
            std::unique_ptr<OutputStream> streamPtr{new FileOutputStream{std::move(*voxStream)}};
            instance.secondaryVoxelSink = IVoxelSink::fromVoxelio(std::move(streamPtr), FileType::MAGICA_VOX, instance.outputResolution);
        }
    }
