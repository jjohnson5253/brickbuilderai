...
Claude to design a model. The asynchronous generation then follows the normal
artifact flow: LDR validation, MPD packing, parts-list creation, Supabase
upload, and `/generation/{id}` polling. It uses `claude-opus-5-5` by default.
Configure it with `ANTHROPIC_LDR_MODEL`, `ANTHROPIC_LDR_MAX_TOKENS` (default
`65536`), and `ANTHROPIC_LDR_TIMEOUT_SECONDS` (default `600`, per Claude call).
It shares `ANTHROPIC_API_KEY` and optional `ANTHROPIC_WORKSPACE_ID` with
`/llmRender`.

By default (`CLAUDE_LDR_MODE=design`) Claude does not write LDraw coordinates.
It submits a voxel design (boxes, ellipsoids, cylinders and per-layer pixel
maps on a stud grid, in brick or plate layers, using only colors from
`gobrick_colors.csv`) and `src/utils/brick_design.py` builds it:
...
5. report loose bricks, weak joints and separate pieces back to Claude, with
...
Claude as tool errors (`CLAUDE_DESIGN_MAX_ATTEMPTS`, default `3`); on the last
attempt unsupported bricks are recolored or dropped instead of failing. Claude
then reviews the renders and accepts or revises (`CLAUDE_DESIGN_REVIEW_ROUNDS`,
default `1`; `0` skips the review). `detail_level` is used as the target size in
studs (clamped to 12-64).

`CLAUDE_LDR_MODE=direct` keeps the original raw-LDraw path. Its output is now
audited with `audit_ldraw` (overlaps, off-grid and floating bricks/plates) and
Claude gets `CLAUDE_DIRECT_FIX_ROUNDS` (default `1`) chances to correct it.
```bash
